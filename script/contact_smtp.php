...
/* ReCaptch Secret */
$recaptchaSecret = '6Lcrn5soAAAAAGMNifCX3mdCdw7_HoefE-n_qx1i';

$dzEmailTo 		= "manager@smiletechci.com";   /* Receiver Email Address */
...
function smtp_mail($dzEmailTo, $dzEmailFrom, $dzEmail, $dzMailSubject, $dzMailMessage)
{
    ...
    $mail->Host       = 'mail.smiletechci.com';                						//Set the SMTP server to send through
    $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
    $mail->Username   = 'manager@smiletechci.com';                   					//SMTP username
    $mail->Password   = 'MANAGERsmile2023';                       				//SMTP password
...
try {
    if (!empty($_POST)) {
		...
		if($_POST['dzToDo'] == 'Contact')
		{
			...
			{
				$dzRes['status'] = 1;
				$dzRes['msg'] = 'Votre Mail a bien été envoyé. Merci de faire confiance à SmileTech!!!';
			}
			else
			{
				$dzRes['status'] = 0;
				$dzRes['msg'] = 'Désolé , nous rencontrons un problème. Réessayer à nouveau';
			}
			echo json_encode($dzRes);
			exit;
		}
